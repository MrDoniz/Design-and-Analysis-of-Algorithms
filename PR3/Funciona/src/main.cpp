...
void ejecutar (RAM &simulador, int contador_programa, Instruccion &instruccion, 
  ...
  try {
    ...
    while (!instruccion.isHalt()) {
      ...
      simulador.ejecutar();
      if (simulador.debug_ > 1){
        instruccion.mostrarInstruccion();
        simulador.entrada_.mostrar();
        simulador.salida_.mostrar();
      }
...
int main(int argc, char *argv[]) {
  ...
  RAM simulador(memoria, programa, entrada, salida, debug);
